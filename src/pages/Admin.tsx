...
import {
  ...
  getSettings, saveSettings, sendTelegram
} from '../lib/db';
...
const ALL_CATS = [
  'Блюда с мангала','Шашлык на костях','Овощи на мангале','Рыба на мангале','Садж на мангале',
  'Супы','Горячие блюда','Шах плов','Гарниры','Паста',
  'Закуски к пиву','Соусы','Холодные закуски','Салаты',
  'Напитки','Авторские чаи','Мороженое','Десерты',
  'Коктейли','Водка','Виски','Коньяк','Ром','Текила','Вино','Шампанское','Пиво',
];
...
export default function Admin() {
  ...
  const testTg = async () => {
    ...
    await saveSettings(tg as unknown as Record<string, string>);
    await sendTelegram('✅ Соль и Перец: Telegram работает!');
    alert('✅ Если бот настроен верно — сообщение отправлено!');
  };
