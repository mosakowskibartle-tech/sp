...
export default function Reserve() {
  ...
  // Tables booked at the EXACT selected time slot
  const bookedAtTime: number[] = (time
    ? dayReservations.filter(r => r.time === time)
    : []
  ).map(r => r.table_number as number);

  // All tables with ANY booking today (shown lighter)
  const bookedAnyTime: number[] = [...new Set(dayReservations.map(r => r.table_number as number))];

  // What to pass to FloorPlan: if time selected → exact conflicts; else all-day bookings
  const bookedForPlan = time ? bookedAtTime : bookedAnyTime;
